...
`order`属性控制 flex项目 的顺序。默认顺序就是 flex项目 被添加进 flex容器 的顺序。

    .flex-item {
        -webkit-order: 整数; /* Safari */
        order:         整数;
    }
...
这个属性指定 扩展因子，在分配多余空间时，相对于其他 flex项目 扩展的比例。
（类比 权重）

    .flex-item {
        -webkit-flex-grow: 非负整数; /* Safari */
        flex-grow:         非负整数;
    }
...
这个属性指定 缩小因子，在分配多余空间时，相对于其他 flex项目 缩小的比例。
...
这个属性跟 `width` 和 `height` 具有相同的值，指定初始主轴尺寸。
