...
将 JavaScript 对象视为只是键/值对的哈希映射有助于理解它。 您可以使用点表示法或括号表示法来访问对象属性。 您可以随时通过为其指定值来添加属性，并且可以随时使用 `delete` 运算符删除属性。 您始终可以通过在属性名称和对象上使用 `in` 运算符来检查属性是否存在。 如果有问题的属性是自有属性，您也可以使用 `hasOwnProperty()`，它存在于每个对象上。 默认情况下，所有对象属性都是可枚举的，这意味着它们将出现在 `for-i`n 循环中或可由 `Object.keys*()` 检索到。

有两种类型的属性：数据属性和访问器属性。 数据属性是值的占位符，您可以读取和写入它们。 当数据属性包含函数值时，该属性被视为该对象的方法。 与数据属性不同，访问器属性不会自己存储值; 他们使用 `getter` 和 `setter` 的组合来执行特定的操作。 您可以使用对象字面量直接创建数据属性和访问者属性。

所有属性都有多个关联特性。 这些特性定义属性的工作方式。 数据和访问器属性都具有 `[[Enumerable]]` 和 `[[Configurable]]` 属性。 数据属性还具有 `[[Writable]]` 和 `[[Value]]` 属性，而访问者属性具有 `[[Get]]` 和 `[[Set]]` 属性。 默认情况下，所有属性的 `[[Enumerable]]` 和 `[[Configurable]]` 设置为 `true`，数据属性的 `[[Writable]]` 设置为 `true`。 您可以使用 `Object.defineProperty()` 或 `Object.defineProperties()` 更改这些属性。 也可以使用 `Object.getOwnPropertyDescriptor()` 来检索这些属性。

当您想以某种方式锁定对象的属性时，有三种不同的方法可以执行此操作。 如果使用 `Object.preventExtensions()`，则对象将不再允许添加属性。 您还可以使用 `Object.seal()` 方法创建一个密封对象，这会使该对象不可扩展，并使其属性不可配置。 `Object.freeze()` 方法创建一个冻结对象，它是一个具有不可写数据属性的密封对象。 小心不可扩展的对象，并始终使用严格模式，以便错误地访问对象时将引发错误。