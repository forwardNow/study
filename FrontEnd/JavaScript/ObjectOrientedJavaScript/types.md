...
虽然 JavaScript 没有类，但它确实有类型。 每个变量或数据片段与特定的基本类型或参考类型相关联。 五种基本类型（字符串，数字，布尔值，空值和未定义）表示直接存储在给定上下文的变量对象中的简单值。 您可以使用 `typeof` 来标识原始类型，但 `null` 除外，必须直接与特殊值 `null` 进行比较。

引用类型是 JavaScript 中最接近类的，对象是引用类型的实例。 您可以使用 `new` 运算符或对象字面量创建新对象。 您主要使用点表示法访问属性和方法，但您也可以使用方括号表示法。 函数是 JavaScript 中的对象，您可以使用 `typeof` 运算符来识别它们。 您应该使用带有构造函数的 `instanceof` 来标识任何其他引用类型的对象。

为了使基本类型值看起来更像引用类型值，JavaScript 有三种原始包装类型：`String`、`Number`、`Boolean`。 JavaScript 在幕后创建这些对象，以便您可以像处理常规对象一样处理基本类型值，但是一旦使用它们的语句完成，临时对象就会被销毁。 虽然您可以创建自己的包装器实例，但最好不要这样做，因为它可能会令人困惑。